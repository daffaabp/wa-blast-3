...
export class MessageService {
	...
	private getRandomDelay(
		isPerFiveMessages = false,
		isPerThirtyMessages = false,
		isPerOneHundredFiveMessages = false,
	): number {
		if (isPerOneHundredFiveMessages) {
			return Math.floor(
				Math.random() *
					(config.MAX_DELAY_PER_105_MESSAGES -
						config.MIN_DELAY_PER_105_MESSAGES +
						1) +
					config.MIN_DELAY_PER_105_MESSAGES,
			);
		}
		if (isPerThirtyMessages) {
			return Math.floor(
				Math.random() *
					(config.MAX_DELAY_PER_30_MESSAGES -
						config.MIN_DELAY_PER_30_MESSAGES +
						1) +
					config.MIN_DELAY_PER_30_MESSAGES,
			);
		}
	...
	private async delay(messageCount: number) {
		...
		const isPerThirtyMessages = messageCount > 0 && messageCount % 30 === 0;
		const isPerOneHundredFiveMessages =
			messageCount > 0 && messageCount % 105 === 0;
		const delayTime = this.getRandomDelay(
			isPerFiveMessages,
			isPerThirtyMessages,
			isPerOneHundredFiveMessages,
		);

		if (isPerOneHundredFiveMessages) {
			console.log(
				`Menunggu ${delayTime / 1000} detik setelah mengirim 105 pesan...`,
			);
		} else if (isPerThirtyMessages) {
			console.log(
				`Menunggu ${delayTime / 1000} detik setelah mengirim 30 pesan...`,
			);
		} else if (isPerFiveMessages) {
			console.log(
